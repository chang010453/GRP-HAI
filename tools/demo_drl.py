...
from model.factory import run_GRP_HAI, print_timings, get_image_blob
...
def parse_args():
  ...
  """
    ...
    parser = argparse.ArgumentParser(description='Test a GRP-HAI network')
    parser.add_argument('--cfg', dest='cfg_file',
                        help='optional config file', default='experiments/cfgs/GRP-HAI-P4.yml', type=str)
    parser.add_argument('--model', dest='model',
                        help='model to test',
                        default='/media/data/GRP-HAI/experiments/drl-model-2/P4/res101/drl-model-2-1/output/res101_GRP_HAI/cell_train/res101_GRP_HAI_iter_110000.ckpt',
                        type=str)
    ...
    parser.add_argument('--nbr_fix', dest='nbr_fix',
                        help='0: auto-stop, > 0 run GRP-HAI exactly nbr_fix steps',
                        default=0, type=int)
...
def im_detect(sess, net, im, timers, im_idx=None):
    ...
    scores, pred_bboxes, timers, _ \
        = run_GRP_HAI(sess, net, blobs, timers, 'test', cfg.GRP_HAI_TEST.BETA,
                      im_idx, alpha=cfg.GRP_HAI.ALPHA)
...
if __name__ == '__main__':
    ...
    logger = setup_logger("GRP-HAI", save_dir=output_dir, filename="log_test.txt")
    ...
    if args.alpha:
        cfg_from_list(['GRP_HAI.ALPHA', True])
    else:
        cfg_from_list(['GRP_HAI.ALPHA', False])

    # VISUALIZE IMAGE
    cfg_from_list(['GRP_HAI_TEST.DO_VISUALIZE', True])

    # Specify if run GRP-HAI in auto mode or a fix number of iterations
    cfg_from_list(['GRP_HAI_TEST.NBR_FIX', args.nbr_fix])
    ...
    net.build_GRP_HAI_network(False)
    ...
    do_visualize = cfg.GRP_HAI_TEST.DO_VISUALIZE

    load_dir = "demo_image"
    imgs = os.listdir(load_dir)
    _t = {'im_detect': Timer(), 'misc': Timer(), 'total_time': Timer()}
    _t_GRP_HAI = {'init': Timer(), 'fulltraj': Timer(),
    ...
    for i in range(len(imgs)):
        ...
        _t['im_detect'].tic()
        scores, boxes, _t_GRP_HAI, _ = im_detect(sess, net, im, _t_GRP_HAI,
                                                     im_idx)
        _t['im_detect'].toc()

        logger.info('\nim_detect: {:d}/{:d} {:.3f}s'.format(i + 1, len(imgs), _t['im_detect'].average_time))
        # print_timings(_t_GRP_HAI) # uncomment for some timing details!
