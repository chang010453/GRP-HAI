...
SAVE_PATH=/media/data/GRP-HAI/experiments/change_det/${DATASET}/${NET}/change_det/
WEIGHTS_PATH=/media/data/GRP-HAI/fr-rcnn-weights/${NET}/${TRAIN_IMDB}/default/${NET}_faster_rcnn_iter_180000.ckpt
if [[ ! -z  ${EXTRA_ARGS_SLUG}  ]]; then
  CUDA_VISIBLE_DEVICES=${GPU_ID} time python ./tools/trainval_net_drl.py \
    ...
    --imdbval ${TEST_IMDB} \
    --iters ${ITERS} \
    --cfg experiments/cfgs/GRP-HAI-${NET}.yml \
    --tag ${EXTRA_ARGS_SLUG} \
    --net ${NET} \
    --det_start ${DET_START} \
    --alpha ${ALPHA} \
