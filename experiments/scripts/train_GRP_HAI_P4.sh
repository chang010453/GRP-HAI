#!/bin/bash
#./experiments/scripts/train_GRP_HAI.sh 0 pascal_voc_0712 20000 110000 True
set -x
set -e
...
SAVE_PATH=/media/data/GRP-HAI/experiments/change_det/${DATASET}/P4/${NET}/change_det-1/
WEIGHTS_PATH=/media/data/drl-rpn/fr-rcnn-weights/P4/res101/${TRAIN_IMDB}/default/res101_faster_rcnn_iter_180000.ckpt
...
if [[ ! -z  ${EXTRA_ARGS_SLUG}  ]]; then
  CUDA_VISIBLE_DEVICES=${GPU_ID} time python ./tools/trainval_net_drl.py \
    ...
    --imdbval ${TEST_IMDB} \
    --iters ${ITERS} \
    --cfg experiments/cfgs/GRP-HAI-P4.yml \
    --tag ${EXTRA_ARGS_SLUG} \
    --net ${NET} \
    --det_start ${DET_START} \
    --alpha ${ALPHA} \
    --set ANCHOR_SIZES ${ANCHOR_SIZES} ANCHOR_STRIDES ${ANCHOR_STRIDES} ANCHOR_RATIOS ${ANCHOR_RATIOS} \
          ...
          GRP_HAI_TRAIN.STEPSIZE ${GRP_HAI_STEPSIZE} ${EXTRA_ARGS}
else
  CUDA_VISIBLE_DEVICES=${GPU_ID} time python ./tools/trainval_net_drl.py \
    --weight ${WEIGHTS_PATH} \
    --save ${SAVE_PATH} \
