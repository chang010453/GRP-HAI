...
from layer_utils.proposal_layer import proposal_layer_all
...
class Network(object):
    ...
    def _proposal_layer_all(self, rpn_bbox_pred, name='proposal_all'):
        with tf.variable_scope(name) as scope:
    ...
    def _anchor_component(self):
        with tf.variable_scope('ANCHOR_' + self._tag) as scope:
            ...
            height = tf.to_int32(tf.ceil(self._im_info[0] / np.float32(self._anchor_strides[0])))
            width = tf.to_int32(tf.ceil(self._im_info[1] / np.float32(self._anchor_strides[0])))
    ...
    ############# GRP-HAI ADDITIONAL COMPONENTS -- START #########################

    def train_GRP_HAI(self, sess, lr_rl, sc, stats):
        # Compute baseline
        if cfg.GRP_HAI_TRAIN.USE_BL:
            # cfg.GRP_HAI.MAX_ITER_TRAJ = 13
            bl_means_total = np.empty(cfg.GRP_HAI.MAX_ITER_TRAJ)
            bl_stds_total = np.empty(cfg.GRP_HAI.MAX_ITER_TRAJ)
            for idx in range(cfg.GRP_HAI.MAX_ITER_TRAJ):
        ...
        curr_batch_avg_loss /= cfg.GRP_HAI_TRAIN.BATCH_SIZE
        curr_batch_ce_done_rew_prod /= cfg.GRP_HAI_TRAIN.BATCH_SIZE
        curr_batch_ce_fix_rew_prod /= cfg.GRP_HAI_TRAIN.BATCH_SIZE
    ...
    def _collect_traj(self, t, free_will, nbr_gts):
        ...
        for t in reversed(range(0, len(ep_total_reward))):
            running_add = running_add * cfg.GRP_HAI_TRAIN.P_GAMMA + ep_total_reward[t]
    ...
    def reset_after_gradient(self):
        ...
        self._bl_total = [[] for _ in range(cfg.GRP_HAI.MAX_ITER_TRAJ)]
    ...
    def _build_network(self, is_training=True):
        ...
        # to use that conv-map again (e.g. when training detector in GRP-HAI
        ...
        reg_prop_in = tf.cond(tf.equal(self._cond_switch, 0),
                              lambda: self._net_conv_from_im(),
                              lambda: self._net_conv_given())
        reg_prop_in = self._net_conv_from_im()

        with tf.variable_scope(self._scope, self._scope):
            # build the anchors for the image
            self._anchor_component()
            # region proposal network (we use the bbox and cls rpn outputs in GRP-HAI)
        ...
        if cfg.POOLING_MODE == 'roi_pooling':
            pool5_GRP_HAI = self._crop_pool_layer(self._net_conv_in, rois_in, self._im_info, "pool5_GRP_HAI")
        elif cfg.POOLING_MODE == 'roi_align':
            pool5_GRP_HAI = roi_align(self._net_conv_in, rois_in, self._anchor_strides[0], cfg.POOLING_SIZE)
        else:
            raise NotImplementedError

        fc7_seq = self._head_to_tail(pool5_GRP_HAI, is_training)
        with tf.variable_scope(self._scope, self._scope):
            self._region_classification(fc7_seq, is_training, initializer,
                                        initializer_bbox)

    def build_GRP_HAI_network(self, is_training=True):
    ...
    def HAI(self, is_training, initializer, name='conv_gru'):
        ...
        self._rl_in = tf.placeholder(tf.float32, shape=[None, None, None, dims_base + dims_aux])
    ...
    def HAI_rollout(self, i, done_logits_all, fix_logits_all, done_prob, h, fix_prob_map):
        ...
        # eq. (1)
        # size=(1,h,w,240+60)
        xr_conv = tf.concat([self.conv(attention_outputs_reshape, self.xr_kernel_base),
        ...
        # eq. (2)
        # size=(1,h,w,240+60)
        xh_conv = tf.concat([self.conv(attention_outputs_reshape, self.xh_kernel_base),
        ...
        # eq. (3)
        # size=(1,h,w,240+60)
        xz_conv = tf.concat([self.conv(attention_outputs_reshape, self.xz_kernel_base),
    ...
    def _compute_rpn_bbox_norm(self, rpn_bbox_pred, name='rpn_bbox_norms'):
        ...
        maximum = tf.reduce_max(rpn_bbox_format, axis=0)
        # normalization
        rpn_bbox_norm = rpn_bbox_format / maximum

        # reshape back to original format
        rpn_bbox_norm_out = tf.reshape(rpn_bbox_norm, input_shape)
    ...
    def _initial_rl_input(self, net_conv, rpn_cls_prob, rpn_bbox_pred,
        ...
        rl_in_init = tf.concat([net_conv / tf.reduce_max(net_conv), rpn_cls_objness, rpn_bbox_norm], 3)  # h*w*530
        self._predictions['rl_in_init'] = rl_in_init

        # Also setup all RoIs and RoI observation volume
        self._proposal_layer_all(rpn_bbox_pred)
    ...
    def get_init_rl(self, sess, image, im_info):
        ...
        net_conv, rl_in, rois_all, roi_obs_vol, not_keep_ids \
        ...
        # Create GRP-HAI hidden state (conv-GRU hidden state)
        ...
        fix_rect_h = int(round(cfg.GRP_HAI.H_FIXRECT * height))
        fix_rect_w = int(round(cfg.GRP_HAI.W_FIXRECT * width))

        # Return
        return net_conv, rl_in, rl_hid, rois_all, roi_obs_vol, \
               height, width, fix_rect_h, fix_rect_w, not_keep_ids

    def action_pass(self, sess, rl_in, rl_hid, is_training=True):
        """ This is the "forward pass" of the GRP-HAI action selection """
    ...
    """
        ...
        # If test-time (or any time, e.g. GRP-HAI training, where we are NOT
    ...
    ############# GRP-HAI ADDITIONAL COMPONENTS -- END ##########################
