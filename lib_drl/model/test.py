...
def im_detect(sess, net, im, timers, im_idx=None, nbr_gts=None):
    ...
    scores, pred_bboxes, timers, stats \
        = run_GRP_HAI(sess, net, blobs, timers, 'test', cfg.GRP_HAI_TEST.BETA,
                      im_idx, nbr_gts, alpha=cfg.GRP_HAI.ALPHA)

    return scores, pred_bboxes, timers, stats


def test_net(sess, net, imdb, weights_filename, max_per_image=100, thresh=0.00):
    """Test a GRP-HAI network on an image database."""
    logger = logging.getLogger("GRP-HAI.test_net")
    ...
    # Create StatCollector (tracks various GRP-HAI test statistics)
    ...
    _t['total_time'].tic()
    # Test GRP-HAI on the test images
    for i in range(start_idx, end_idx):
        ...
        _t['im_detect'].tic()
        scores, boxes, _t_GRP_HAI, stats = im_detect(sess, net, im, _t_GRP_HAI,
                                                     im_idx, nbr_gts)
        _t['im_detect'].toc()
