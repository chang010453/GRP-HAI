...
from model.factory import run_GRP_HAI
import logging
...
class SolverWrapper(object):
    ...
    def construct_graph(self, sess):
        ...
        with sess.graph.as_default():
            ...
            # Initialize main GRP-HAI network
            self.net.build_GRP_HAI_network()
    ...
    def initialize(self, sess):
        ...
        drl_rate = cfg.GRP_HAI_TRAIN.LEARNING_RATE
        drl_stepsize = cfg.GRP_HAI_TRAIN.STEPSIZE
    ...
    def restore(self, sess, sfile, nfile):
        ...
        if last_snapshot_iter > drl_stepsize:
            drl_rate *= cfg.GRP_HAI_TRAIN.GAMMA
    ...
    def _check_if_continue(self, iter, max_iters, snapshot_add):
        img_start_idx = cfg.GRP_HAI_TRAIN.IMG_START_IDX
        ...
        if iter == img_start_idx:
            ...
            iter = 0
            cfg_from_list(['GRP_HAI_TRAIN.IMG_START_IDX', -1])
            cfg_from_list(['GRP_HAI_TRAIN.DET_START',
                           cfg.GRP_HAI_TRAIN.DET_START - img_start_idx])
            cfg_from_list(['GRP_HAI_TRAIN.STEPSIZE',
                           cfg.GRP_HAI_TRAIN.STEPSIZE - img_start_idx])
    ...
    def train_model(self, sess, max_iters):
        ...
        # Sample first beta
        beta = cfg.GRP_HAI_TRAIN.BETA

        # Setup GRP-HAI timers
        timers = {'init': Timer(), 'fulltraj': Timer(), 'upd-obs-vol': Timer(),
                  'upd-seq': Timer(), 'upd-rl': Timer(), 'action-rl': Timer(),
                  'coll-traj': Timer(), 'run-GRP-HAI': Timer(),
                  'train-GRP-HAI': Timer(), 'batch_time': Timer(), 'total': Timer()}
        ...
        sc = StatCollector(max_iters, stat_strings, cfg.GRP_HAI_TRAIN.BATCH_SIZE, self.output_dir)
        ...
        while iter < max_iters:
            ...
            if do_continue:
                continue
            # Potentially update GRP-HAI learning rate
            # 90000
            if (iter + 1) % cfg.GRP_HAI_TRAIN.STEPSIZE == 0:
                # lr_rl = lr_rl * 0.2
                lr_rl *= cfg.GRP_HAI_TRAIN.GAMMA

            # Run GRP-HAI in training mode
            timers['run-GRP-HAI'].tic()
            net_conv, rois_GRP_HAI, gt_boxes, im_info, timers, stats = run_GRP_HAI(sess, self.net, blobs, timers, mode='train',
                                beta=beta, im_idx=None, extra_args=lr_rl, alpha=cfg.GRP_HAI.ALPHA)
            timers['run-GRP-HAI'].toc()

            # BATCH_SIZE = 50
            if (iter + 1) % cfg.GRP_HAI_TRAIN.BATCH_SIZE == 0:
                self.logger.info("\n##### GRP-HAI BATCH GRADIENT UPDATE - START ##### \n")
                self.logger.info('iter: %d / %d' % (iter + 1, max_iters))
                self.logger.info('lr-rl: %f' % lr_rl)
                timers['train-GRP-HAI'].tic()
                self.net.train_GRP_HAI(sess, lr_rl, sc, stats)
                timers['train-GRP-HAI'].toc()
                sc.print_stats(iter=iter+1, logger=self.logger)

                batch_time = timers['batch_time'].toc()
                self.logger.info('TIMINGS:')
                self.logger.info('runnn-GRP-HAI: %.4f' % timers['run-GRP-HAI'].get_avg())
                self.logger.info('train-GRP-HAI: %.4f' % timers['train-GRP-HAI'].get_avg())
                self.logger.info('train-GRP-HAI-batch: %.4f' % batch_time)
                self.logger.info("\n##### GRP-HAI BATCH GRADIENT UPDATE - DONE ###### \n")
            ...
            # We next train detector with GRP-HAI running in deterministic mode.
            # Potentially train detector component of network
            # DET_START = 20000
            if 0 <= cfg.GRP_HAI_TRAIN.DET_START <= iter:

                # Run GRP-HAI in deterministic mode
                # net_conv, rois_GRP_HAI, gt_boxes, im_info, timers \
                #     = run_GRP_HAI(sess, self.net, blobs, timers, mode='train_det',
                #                   beta=beta, im_idx=None, alpha=cfg.GRP_HAI.ALPHA)
                ...
                if rois_GRP_HAI is not None:
                    timer.tic()
                    # Train detector part
                    loss_cls, loss_box, tot_loss \
                        = self.net.train_step_det(sess, train_op, net_conv, rois_GRP_HAI,
                                                gt_boxes, im_info)
                    timer.toc()
...
def train_net(network, imdb, roidb, valroidb, output_dir,
    ...
    """Train GRP-HAI for a Faster R-CNN network."""
    ...
    logger = logging.getLogger("GRP-HAI.train_net")
