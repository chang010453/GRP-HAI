...
def update_obs_vol(roi_obs_vol, t, height, width, fix_h, fix_w, obs_rect_h, obs_rect_w):
    ...
    h_start, w_start, h_end, w_end \
        = _get_rect_coords(fix_h, fix_w, obs_rect_h, obs_rect_w, height, width)
...
def _check_termination(t, done_prob, mode='train'):
    ...
    if mode == 'train':
        ...
        # False
        random_done = cfg.GRP_HAI_TEST.RANDOM_DONE

    # Basic termination
    if mode == 'test' and cfg.GRP_HAI_TEST.NBR_FIX > 0:
        if t + 1 == cfg.GRP_HAI_TEST.NBR_FIX:
            return True, False
        return False, False
    else:
        if random_done:
    ...
    if t == cfg.GRP_HAI.MAX_ITER_TRAJ - 1:
...
def sample_fix_loc(fix_prob, mode='train'):
    ...
    if mode == 'train':
        ...
        random_fix = cfg.GRP_HAI_TEST.RANDOM_FIX
...
# Run GRP-HAI detector on an image blob
def run_GRP_HAI(sess, net, blob, timers, mode, beta, im_idx=None,
    ...
    # Run initial GRP-HAI processing (get base feature map etc)
    timers['init'].tic()
    net_conv, rl_in, rl_hid, rois_all, roi_obs_vol, height, width, fix_rect_h, \
    fix_rect_w, not_keep_ids_agno_nms = net.get_init_rl(sess, im_blob, im_info)
    ...
    # where GRP-HAI attention has been placed in the trajectory
    ...
    for t in range(cfg.GRP_HAI.MAX_ITER_TRAJ):
        # Update RL state
        if t > 0:
            ...
            if np.count_nonzero(roi_obs_vec_seq) > 0:
                ...
                # cls_probs_seq: size(N, 21), bbox_preds_seq: size(N, 21*4)
...
def save_visualization(im_blob, im_shape, im_idx, obs_canvas, cls_probs,
    ...
    for i in range(obs_canvas.shape[2]):
        ...
        if show_text:
            ax.text(start_x, start_y, "g" + str(i + 1), color='black', weight='bold',
                    fontsize=8,
        ...
        for j in range(len(names_and_coords)):
            ...
            if show_text:
                ax.text(coords[0], coords[1], name + " " + str(score),
                        weight='bold', color='black', fontsize=10,
                        horizontalalignment='center', verticalalignment='center',
                        bbox=dict(facecolor='white', edgecolor='white', pad=-0.1))
    ...
    if ~show_all_steps:
        ...
        im_name = 'im' + str(im_idx + 1) + '.png'
        # fig.savefig(im_name, transparent=True, dpi=300, bbox_inches='tight', pad_inches=0.0)
        plt.savefig(im_name, transparent=True, bbox_inches='tight', pad_inches=0.0, dpi=300)
    plt.close()
