...
#
# Training options (for Faster R-CNN, but see GRP-HAI a few 100 lines down)
#
__C.TRAIN = edict()
...
__C.TRAIN.SNAPSHOT_PREFIX = 'vgg16_GRP_HAI'
...
#
# Testing options (for Faster R-CNN, but see GRP-HAI a few 10's of lines down)
#
__C.TEST = edict()
...
# TRAINING
__C.GRP_HAI_TRAIN = edict()

# Learning rate etc
__C.GRP_HAI_TRAIN.LEARNING_RATE = 0.00002
__C.GRP_HAI_TRAIN.GAMMA = 0.20  # Factor for reducing the learning rate
__C.GRP_HAI_TRAIN.STEPSIZE = 90000
__C.GRP_HAI_TRAIN.DISPLAY = 50
__C.GRP_HAI_TRAIN.BATCH_SIZE = 50

# Use horizontally-flipped images during training?
__C.GRP_HAI_TRAIN.USE_FLIPPED = True

# When start training of detector (-1: never train detector, >=0: train from
# that onwards)?
__C.GRP_HAI_TRAIN.DET_START = -1

# Can skip a number of images in training if desired (e.g. if starting
# from a later checkpoint file)
__C.GRP_HAI_TRAIN.IMG_START_IDX = -1

# Fixation penalty(ies) used in training
__C.GRP_HAI_TRAIN.BETA = 0.05

# Use baseline?
__C.GRP_HAI_TRAIN.USE_BL = True

# Moving average factor
__C.GRP_HAI_TRAIN.MA_WEIGHT = 0.0005

# gamma
__C.GRP_HAI_TRAIN.P_GAMMA = 0.95

# IoU-thresholds in rewards
__C.GRP_HAI_TRAIN.IOU_THRESH = 0.5

# TESTING
__C.GRP_HAI_TEST = edict()

# Randomness during testing
__C.GRP_HAI_TEST.RANDOM_DONE = False
__C.GRP_HAI_TEST.RANDOM_FIX = False

# Fixation penalty beta
__C.GRP_HAI_TEST.BETA = 0.05

# Run a certain number of fixations in GRP-HAI?
__C.GRP_HAI_TEST.NBR_FIX = 0  # 0 = automatic stopping

# Visualize GRP-HAI search trajectories?
__C.GRP_HAI_TEST.DO_VISUALIZE = False

# GENEREAL
__C.GRP_HAI = edict()

# Use attentin's alpha as fixate location
__C.GRP_HAI.ALPHA = False

# Fixation rectangle size
__C.GRP_HAI.H_FIXRECT = .25
__C.GRP_HAI.W_FIXRECT = .25

# Max iterations per trajectory used in training
__C.GRP_HAI.MAX_ITER_TRAJ = 13
__C.GRP_HAI.MAX_ITER_TRAJ_FLT = float(__C.GRP_HAI.MAX_ITER_TRAJ)
...
__C.DIMS_AUX = 5 * __C.NBR_ANCHORS
